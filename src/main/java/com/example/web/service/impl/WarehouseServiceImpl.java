...
public class WarehouseServiceImpl extends ServiceImpl<WarehouseMapper, Warehouse> implements WarehouseService {
   ...
   //如果前端搜索传入查询条件则拼接查询条件
        ...
        if (Extension.isNotNullOrEmpty(input.getAnnualTurnover())) {
             queryWrapper = queryWrapper.like(Warehouse::getAnnualTurnover, input.getAnnualTurnover());
       	 }
        ...
        if (Extension.isNotNullOrEmpty(input.getRating())) {
             queryWrapper = queryWrapper.like(Warehouse::getRating, input.getRating());
       	 }
        if (Extension.isNotNullOrEmpty(input.getEmployeesCount())) {
             queryWrapper = queryWrapper.like(Warehouse::getEmployeesCount, input.getEmployeesCount());
       	 }
      return queryWrapper;
    }
